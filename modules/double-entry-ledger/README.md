...
- **Conversion rules:** Configurable conversion matrix (e.g., Silver → Bronze with margin)
...
model Transaction {
  ...
  fromWalletId  String?
  toWalletId    String?
  fromDenom     String?  -- e.g., "SILVER"
  toDenom       String?  -- e.g., "BRONZE"
...
model WalletBalance {
  id            String  @id @default(cuid())
  walletId      String
  denomination  String  -- "GOLD", "SILVER", "BRONZE", "DIAMOND"
...
async function transferP2P(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.walletBalance.upsert({
      where: { walletId_denomination: {...} },
      create: { walletId: toWallet.id, denomination: "BRONZE", amount: BigInt(amount) },
